...
const AdCTAButton: React.FC<{
  ...
  const stylesObj: React.CSSProperties = {
    ...
    ...(ctaPadding !== undefined && { padding: ctaPadding }),
    ...(ctaBorderRadius !== undefined && { borderRadius: ctaBorderRadius }),
  };

  // Map CTA variant to Button kind (add more as needed)
  const variantClass = styles[`advertisement__cta--${ctaVariant}`] || '';
  ...
  return (
    <button className={className} style={stylesObj} onClick={onClick}>
      {ctaText}
    </button>
  );
};
