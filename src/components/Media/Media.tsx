...
const Media = forwardRef<HTMLDivElement, MediaProps>(
	(
		...
		const renderAvatar = () => {
			...
			return (
				<Wrapper className={styles.media__avatar}>
					{displaySrc ?
						<img
							src={displaySrc}
							alt={finalConfig.alt}
						...
						/>
					:	<Wrapper className={styles.media__fallback}>
		...
		const renderImage = () => {
			...
			return (
				<Wrapper className={styles.media__image_container}>
					{displaySrc ?
						<img
							ref={
					...
					{finalConfig.showOverlay && (
						<Wrapper className={styles.media__overlay}>
							{title && (
								<Wrapper
		...
		// Render video types
		const renderVideo = () => (
			<Wrapper className={styles.media__video_container}>
				<video
					ref={
		...
		// Render audio types
		const renderAudio = () => (
			<Wrapper className={styles.media__audio_container}>
				{(kind === 'audio-card' ||
					kind === 'podcast-player') &&
					src && (
						<Wrapper className={styles.media__audio_cover}>
							<img
								src={src}
								alt={finalConfig.alt}
								className={styles.media__image}
		...
		const renderMetadata = () => {
			...
			return (
				<Wrapper className={styles.media__metadata}>
					{metadata.title && (
						<Wrapper
							className={styles.media__metadata_title}
						>
							{metadata.title}
						</Wrapper>
					)}
					...
					<Wrapper
						className={styles.media__metadata_details}
					>
						{metadata.author && (
							<span
					...
					{metadata.tags && metadata.tags.length > 0 && (
						<Wrapper
							className={styles.media__metadata_tags}
						>
		...
		const renderActions = () => {
			...
			return (
				<Wrapper className={styles.media__actions}>
		...
		return (
			<Wrapper
				...
				{...rest}
			>
				<Wrapper className={styles.media__content}>
					{renderMediaContent()}
				</Wrapper>
				...
				{children && (
					<Wrapper className={styles.media__custom}>
						{children}
					</Wrapper>
				)}
			</Wrapper>
