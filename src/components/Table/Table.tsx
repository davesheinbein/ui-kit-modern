...
const Table = forwardRef<HTMLTableElement, TableProps>(
	(props, ref) => {
		...
		const selectedRowsSet = useSelector(
			(state: RootState) =>
				selectTableSelectedRows(state, tableComponentId)
		);
		const expandedRowsSet = useSelector(
			(state: RootState) =>
		...
		const currentFiltersArray: TableFilter[] =
			...
			<thead className={headerClasses}>
				<tr className={styles.table__header_row}>
					...
					{columns.map((column) => (
						...
						>
							<span
								className={styles.table__header_content}
							>
								{column.label}
								{config.sortable &&
									column.sortable !== false && (
					...
					{columns.map((column) => {
						...
						return (
							...
							>
								...
								: config.loading ?
									<Wrapper
										className={styles.skeleton_content}
									/>
								:	cellValue}
							</td>
						);
		...
		const renderSearchBar = () => {
			...
			return (
				<Wrapper className={styles.table__search}>
					<input
						type='text'
						placeholder={
							searchPlaceholder ||
		...
		return (
			<Wrapper className={styles.table_container}>
				{renderSearchBar()}
